...
class EloquentJoins
{
    ...
    {
        Builder::macro('joinRelationship', function ($relation, $callback = null, $joinType = 'join') {
            if (Str::contains($relation, '.')) {
        ...
        Builder::macro('leftJoinRelationship', function ($relation, $callback = null) {
            return $this->joinRelationship($relation, $callback, 'leftJoin');
        });

        Builder::macro('rightJoinRelationship', function ($relation, $callback = null) {
            return $this->joinRelationship($relation, $callback, 'rightJoin');
        });

        Builder::macro('joinNestedRelationship', function ($relations, $callback = null, $joinType = 'join') {
            $relations = explode('.', $relations);
            $latestRelation = null;
